...
export default function EditContact() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/auth/contact/update/${id}`, {
        method: 'PUT',
        headers: {
  ...
  if (fetching) {
    return <div className="p-8 text-sahara-800">Loading contact...</div>;
  }
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-sm text-sahara-800 mb-6">
        ...
        <span>/</span>
        <Link to={`/contacts/${id}`} className="hover:text-sienna-600 transition-colors">{formData.name || 'Contact'}</Link>
        <span>/</span>
        <span className="text-sahara-950 font-medium">Edit</span>
      </div>

      <div className="mb-8">
        <h1 className="font-heading text-4xl text-sahara-950 mb-2">Edit Contact</h1>
        <p className="text-sahara-800 text-sm">Update the details of this contact below.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sahara border border-sahara-300/50">
          ...
          <textarea name="notes" value={formData.notes} onChange={handleChange}
            placeholder="Add any personal notes about this contact..." rows={4}
      ...
      <Modal isOpen={duplicateModal.open} onClose={() => setDuplicateModal({ open: false, contact: null })} title="Duplicate Contact Detected">
        ...
        <div className="flex justify-end">
          <button onClick={() => setDuplicateModal({ open: false, contact: null })}
